...
export function getLeftSidebarOpen() { return _leftSidebarOpen; }
export function setLeftSidebarOpen(v) { _leftSidebarOpen = v; }
export function getAgentRoster() { return _agentRoster; }
...
export function initLeftSidebar() {
  ...
  if (closeBtn) closeBtn.onclick = () => { _leftSidebarOpen = false; document.getElementById('left-sidebar')?.classList.remove('open'); };
}
...
export function renderAgentRoster() {
  ...
  list.innerHTML = sorted.map(([name, e]) => {
    ...
    const idle = e.status === 'idle' && e.idleSince ? formatDuration(Math.floor((Date.now() - e.idleSince) / 1000)) : '';
